document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================================
    const CURRENT_VERSION = '2025.12.10.01'; // Thay đổi số này mỗi khi deploy
    ...
    const defaultState = {
      ...
      activeTab: 'settings',
      modes: {
        default: { pairs: [], matchCase: false, wholeWord: false, autoCaps: false, exceptions: 'jpg, png, com, vn, net' }
      }
    };
    ...
    // AUTO RELOAD CHECK (24h or New Version)
    setTimeout(() => location.reload(), 24 * 60 * 60 * 1000);
    setInterval(checkForUpdate, 60 * 1000); // Check version mỗi 1 phút
        ...
        try {
            ...
            if(res.ok) {
                ...
                if(svVer && svVer.trim() !== CURRENT_VERSION) {
                    showNotification("Có bản cập nhật mới! Đang tải lại...", "warning");
    ...
    function updatePairIndexes() {
        ...
        // Đánh số ngược: item đầu tiên là số lớn nhất
        items.forEach((item, index) => {
            item.querySelector('.pair-index').textContent = (total - index) + '.';
        });
    }
    ...
    function importCSV(file) {
        ...
        reader.onload = (e) => {
            ...
            let count = 0;
            const currentPairs = state.modes[state.currentMode].pairs || [];
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (cols.length >= 2) {
                    ...
                    if (find) {
                        ...
                        // Check trùng: Nếu đã có find trong list thì bỏ qua
            ...
            saveState(); renderModeSelect(); loadSettingsToUI(); showNotification(`Đã nhập ${count} cặp mới (bỏ qua trùng)!`);

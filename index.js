document.addEventListener('DOMContentLoaded', () => {
    ...
    // KIỂM TRA QUYỀN VIP (Lấy từ biến toàn cục trong HTML)
    ...
    function performReplaceAll() {
        ...
        if (!isVipUser) {
            ...
            // Chỉ cắt khi bấm nút, còn lúc nhập vẫn đếm đủ
            if (wc > 2000) {
                showNotification("Bản Free: Đã cắt xuống 2000 từ đầu tiên!", "warning");
                // Cắt chính xác số từ
                const words = rawText.trim().split(/\s+/);
                rawText = words.slice(0, 2000).join(" ");
                els.inputText.value = rawText; // Cập nhật lại UI
                updateCounters();
            }
        }

        // Dùng setTimeout để tránh treo trình duyệt với văn bản lớn
        setTimeout(() => {
            try {
                ...
                // [LOGIC FREE: GIỚI HẠN CẶP]
                let pairs = mode.pairs || [];
                if (!isVipUser && pairs.length > 10) {
    ...
    function performSplit() {
        ...
        // [LOGIC FREE: CẮT TỪ CHIA CHƯƠNG]
        if (!isVipUser) {
            const wc = countWords(rawText);
            if (wc > 10000) {
                showNotification("Bản Free: Cắt xuống 10,000 từ đầu tiên!", "warning");
                const words = rawText.trim().split(/\s+/);
                workingText = words.slice(0, 10000).join(" ");
                // Không cập nhật lại UI splitInput để giữ text gốc cho user xem, 
                // hoặc cập nhật nếu muốn user biết bị cắt:
                els.splitInput.value = workingText; 
                updateCounters();
            }
        }
        ...
        // Xử lý bất đồng bộ để tránh treo
        setTimeout(() => {
            if (splitType === 'regex') {
                ...
                // Count Mode Logic
                // [LOGIC FREE: GIỚI HẠN SỐ PHẦN]
                let targetParts = currentSplitMode;
                if (!isVipUser && targetParts > 4) targetParts = 4; // Max 4 phần cho Free
    ...
    function loadSettingsToUI() {
      ...
      const mode = state.modes[state.currentMode];
      if (mode && mode.pairs) {
          // [LOGIC FREE: HIỂN THỊ TỐI ĐA 10 CẶP]
          const pairsToLoad = (!isVipUser && mode.pairs.length > 10) ? mode.pairs.slice(0, 10) : mode.pairs;
    ...
    function importCSV(file) {
        if(!isVipUser) return showNotification("Chức năng chỉ dành cho VIP!", "error");
    ...
    function initEvents() {
      ...
      els.addModeBtn.onclick = () => { 
          if(!isVipUser) return showNotification("VIP mới được thêm Mode!", "error");
      ...
      els.copyModeBtn.onclick = () => {
        if(!isVipUser) return showNotification("VIP mới được sao chép Mode!", "error");
      ...
      els.renameBtn.onclick = () => { 
          if(!isVipUser) return showNotification("VIP mới được đổi tên Mode!", "error");
      ...
      els.deleteBtn.onclick = () => { 
          if(!isVipUser && state.currentMode !== 'default') return showNotification("VIP mới được xóa Mode!", "error");
      ...
      document.querySelectorAll('.split-mode-btn').forEach(btn => btn.onclick = () => { 
          ...
          // Free Limit: Max 4 parts
          if (!isVipUser && val > 4) return showNotification("Bản Free tối đa 4 phần!", "error");
    ...
    // Setup Modal (Check if exists to avoid null error)
